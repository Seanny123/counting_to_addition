# non-voja, basic ensemble, no unitary constraint
# decreases in error, but much more gradually than the associative learning one
...
with model:
    ...
    adder = nengo.Ensemble(n_neurons*8, 2*D)
...
np.savez_compressed("../data/ens_learn_data", t=t, p_keys = sim.data[p_keys], p_values = sim.data[p_values], p_learning = sim.data[p_learning], p_error = sim.data[p_error], p_recall = sim.data[p_recall])
